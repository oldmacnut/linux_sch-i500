...
#include<mach/atlas/gpio-aries-setting.h>
...
static struct regulator_consumer_supply aquila_ldo6_consumers[] = {
#if defined(CONFIG_FB_S3C_MIPI_LCD)
	{
...
static struct regulator_consumer_supply aquila_ldo7_consumers[] = {
	{
	        .dev            = &s5p_device_dsim.dev,
 	        .supply		= "VLCD_1.8V",
	}
...
static void lcd_cfg_gpio(void)
{
	...
	ret = gpio_request(gpio, "MLCD_ON");

	if (ret == 0)
		gpio_export(gpio, 1);
	else
		printk(KERN_ERR "LCD_ON request err %d\n", ret);

	/*
	 *  -- VGH --
	 */
			s3c_gpio_cfgpin(S5PV210_GPH1(7), S3C_GPIO_SFN(0xf));
...
static int lcd_power_on(void *pdev, int onoff)
{
	unsigned int gpio;
	struct device *dev = NULL;

	dev = (struct device *) pdev;
...
static struct regulator_init_data max8998_ldo6_data = {
        ...
        .num_consumer_supplies  = ARRAY_SIZE(aquila_ldo6_consumers),
        .consumer_supplies      = aquila_ldo6_consumers,
};
...
#if defined(CONFIG_S5PV210_GARNETT_DELTA)
	.num_consumer_supplies  = ARRAY_SIZE(aquila_ldo7_consumers),
        .consumer_supplies      = aquila_ldo7_consumers,
...
#if defined(CONFIG_S5PV210_GARNETT_DELTA)
	.num_consumer_supplies  = ARRAY_SIZE(aquila_ldo17_consumers),
        .consumer_supplies      = aquila_ldo17_consumers,
#endif
...
void s3c_config_sleep_gpio(void)
{
...
#if defined(CONFIG_S5PV210_GARNETT_DELTA)
	//s3c_gpio_cfgpin(S5PV210_GPH1(6), S3C_GPIO_OUTPUT);         /* Added for Froyo Atlas */
#else 
	//s3c_gpio_cfgpin(S5PV210_GPH1(6), S3C_GPIO_INPUT);         /* Added for Froyo Atlas */
#endif
    ...
    #if 0 //Open Send end Added for Froyo Atlas
    ...
    #endif 
	s3c_gpio_cfgpin(S5PV210_GPH2(3), S3C_GPIO_OUTPUT);     /* Added for Froyo Atlas */
	s3c_gpio_setpull(S5PV210_GPH2(3), S3C_GPIO_PULL_NONE);
	s3c_gpio_setpin(S5PV210_GPH2(3), 1);                    /* Added for Froyo Atlas */
...
//	s3c_gpio_setpin(S5PV210_GPH3(2), 0);
	...
	s3c_gpio_setpin(S5PV210_GPH3(7), 1);
...
static int ldo8_onoff_state = 0;
#define LDO38_USB_BIT    (0x1)
#define LDO38_TVOUT_BIT  (0x2)
#define LDO8_USB_BIT	(0x3)
...
#ifdef CONFIG_S5PV210_GARNETT_DELTA
static int ldo8_control(int module, bool onoff)
{
    mutex_lock(&ldo38_mutex);
    if (onoff)
    {
        ...
        ldo8_onoff_state |= module;
    }
    else
    {
        printk(KERN_INFO "%s : turn OFF LDO8 (cur_stat=%d, req=%d)\n",__func__,ldo8_onoff_state,module);
        ldo8_onoff_state &= ~module;
        if (!ldo8_onoff_state)
        {
            Set_MAX8998_PM_REG(ELDO8, 0);
...
{
#ifdef CONFIG_S5PV210_GARNETT_DELTA
	/* In Garnett board LDO3 is shared between USB and MIPI */
	ldo8_control(LDO8_USB_BIT, onoff);
#else
